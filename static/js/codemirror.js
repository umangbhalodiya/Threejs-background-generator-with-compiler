...
var CodeMirror = (function () {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    function keyMightStartIME(keyCode) {
      return (
        (win &&
          ((gecko && keyCode == 229) || (window.opera && keyCode == 197))) ||
        (mac && gecko)
      );
    }
    ...
    function onKeyDown(e) {
      ...
      var mod = (mac ? e.metaKey : e.ctrlKey) && !e.altKey,
        anyMod = e.ctrlKey || e.altKey || e.metaKey;
    ...
    function readInput() {
      ...
      if (changed) {
        ...
        while (
          start < len &&
          (c = text.charAt(start)) == editing.text.charAt(start)
        ) {
          ++start;
          if (c == "\n") {
            line++;
        ...
        updateLines(
          ...
          splitLines(text.slice(start, end)),
          from,
          to
        );
    ...
    function prepareInput() {
      ...
      var from = Math.max(0, sel.from.line - 1),
        to = Math.min(doc.size, sel.to.line + 2);
    ...
    function updateCursor() {
      ...
      inputDiv.style.top =
        Math.max(Math.min(globalY, scroller.offsetHeight), 0) + "px";
    ...
    function scrollPage(down) {
      var linesPerPage = Math.floor(scroller.clientHeight / textHeight()),
        head = sel.inverted ? sel.from : sel.to;
      var target =
        heightAtLine(doc, head.line) +
    ...
    function selectAll() {
      ...
      setSelection(
        { line: 0, ch: 0 },
        { line: endLine, ch: getLine(endLine).text.length }
      );
    }
    ...
    function handleEnter() {
      ...
      if (options.enterMode != "flat")
        indentLine(
          sel.from.line,
          options.enterMode == "keep" ? "prev" : "smart"
        );
    }
    ...
    function measureLine(line, ch) {
      ...
      measure.innerHTML =
        "<pre>" +
        line.getHTML(null, null, false, ch) +
    ...
    function textHeight() {
      ...
      measure.innerHTML =
        "<pre>x<br>x<br>x<br>x<br>x<br>x<br>x<br>x<br>x<br>x</pre>";
    ...
    function SearchCursor(query, pos, caseFold) {
      ...
      if (typeof query != "string")
        ...
        this.matches = function (reverse, pos) {
          if (reverse) {
            var line = getLine(pos.line).text.slice(0, pos.ch),
            ...
            var line = getLine(pos.line).text.slice(pos.ch),
              match = line.match(query),
        ...
        if (target.length == 1)
          this.matches = function (reverse, pos) {
            var line = fold(getLine(pos.line).text),
              len = query.length,
              match;
            if (
              reverse
    ...
    replace: function (from, to_, text) {
      ...
      if (mk) {
        ...
        for (var i = 0; i < mk.length; ++i) {
          ...
          if (
            del ||
            (mark.from != null && mark.to != null && mark.from >= mark.to)
          )
            mk.splice(i--, 1);
        }
      }
      ...
      if (mk) {
        for (var i = 0; i < mk.length; ++i) {
          ...
          if (mark.to > pos || mark.to == null) {
            ...
            taken.marked.push({
              from:
                mark.from < pos || mark.from == null
                  ? null
                  : mark.from - pos + textBefore.length,
  ...
  var movementKeys = {};
  for (var i = 35; i <= 40; ++i) movementKeys[i] = movementKeys["c" + i] = true;
